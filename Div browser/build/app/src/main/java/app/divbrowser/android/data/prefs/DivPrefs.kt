...
/**
 * Local preferences — all toggles persist (DataStore).
 */
class DivPrefs(private val context: Context) {
    ...
    private val jsKey = booleanPreferencesKey("js_enabled")
    private val popupsKey = booleanPreferencesKey("block_popups")
    private val clearExitKey = booleanPreferencesKey("clear_on_exit")
    private val defaultPanesKey = intPreferencesKey("default_panes")

    val darkMode: Flow<Boolean> = context.divDataStore.data.map { it[darkKey] ?: true }
    val jsEnabled: Flow<Boolean> = context.divDataStore.data.map { it[jsKey] ?: true }
    val blockPopups: Flow<Boolean> = context.divDataStore.data.map { it[popupsKey] ?: true }
    val clearOnExit: Flow<Boolean> = context.divDataStore.data.map { it[clearExitKey] ?: false }
    /** 1 = single browser, 2–4 = multi-pane default from Settings */
    val defaultPanes: Flow<Int> = context.divDataStore.data.map {
        (it[defaultPanesKey] ?: 1).coerceIn(1, 4)
    }
    ...
    suspend fun setJsEnabled(enabled: Boolean) {
        context.divDataStore.edit { it[jsKey] = enabled }
    }

    suspend fun setBlockPopups(enabled: Boolean) {
        context.divDataStore.edit { it[popupsKey] = enabled }
    }

    suspend fun setClearOnExit(enabled: Boolean) {
        context.divDataStore.edit { it[clearExitKey] = enabled }
    }

    suspend fun setDefaultPanes(count: Int) {
        context.divDataStore.edit { it[defaultPanesKey] = count.coerceIn(1, 4) }
    }
