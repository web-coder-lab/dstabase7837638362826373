...
object DivApi {
    ...
    suspend fun health(): JSONObject? {
        getJson(coreBase, "/health")?.let { return it }
        getJson(coreBase, "/v1/health")?.let { return it }
        // Artists-studio live health (until dedicated /v1 routes are mounted)
        getJson(coreBase, "/api/v1/health")?.let { return it }
        getJson("https://div-store.onrender.com", "/api/health")?.let { return it }
        return null
    }
